...
# Function to fetch the latest frame from the IP Webcam
def fetch_frame_from_ip_webcam(ip_webcam_url):
    try:
        ...
        print(f"Error fetching frame from IP Webcam: {e}")
        return None

# Main function for continuous monitoring using IP Webcam capture
def main():
    ip_webcam_url = 'http://192.168.14.41:8080/shot.jpg'  # IP Webcam snapshot URL
    ...
    while True:
        # Fetch the frame from IP Webcam
        frame = fetch_frame_from_ip_webcam(ip_webcam_url)

        if frame is None:
            print("Failed to fetch frame from IP Webcam.")
            continue
        ...
        cv2.imshow("IP Webcam Attendance Monitoring", frame)
